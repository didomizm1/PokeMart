<!DOCTYPE html>
<html>
<head>
        <title> WALLET </title>
</head>    
<body style="background-image: radial-gradient(circle, rgb(195, 235, 242) 40%, rgb(0, 180, 245) 180%);" >
    <center>     
        <IMG SRC="../../img/card/My-Wallet.png" >
    <body>
        <br><br>
            <label> 
            <div>
                <IMG SRC="../../img/card/Saved-Payments.png" >
             <br><br>
            <form action="search_card.php" method="POST"> 
             <input type="submit" name="submit">
            </form>
            </div>
            </label>
    </body>
    <br><br>
    <label> 
    <div>
        <font face="Arial" size="3px">    
             <a href="http://localhost:8080/PokeMart/forms/card_forms/add_card.php"> Add New Payment </a>
       </font> 
     </div>
    <br>
    <div>
        <a href="http://localhost:8080/PokeMart/forms/card_forms/cont.php" >
            <button>Use This Payment Method</button>
        </a>
    </div>
    </label>
</center>
   </p>
</body>
</html>